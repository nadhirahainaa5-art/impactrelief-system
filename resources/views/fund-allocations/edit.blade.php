...
@section('title', 'Edit Budget Request')
@section('page_title', 'Edit Budget Request')
...
<div class="budget-hero">
    <div>
        <p class="eyebrow">IMPACTRELIEF BUDGET REVISION</p>
        <h1>Edit Budget Allocation Request</h1>
        <p>
            Revise this budget request and resubmit it for admin approval.
            Once approved, this budget becomes the limit for future expense claims.
        </p>
    </div>

    <a href="{{ route('fund-allocations.show', $fundAllocation) }}" class="back-btn">
        Back to Request
    </a>
</div>
...
<form method="POST"
      action="{{ route('fund-allocations.update', $fundAllocation) }}">
    ...
    <div class="budget-layout">
        ...
        <div class="budget-card">
            <h3 class="section-title">Revision Guidance</h3>

            <div class="control-rule">
                <strong>Important:</strong><br>
                Updating this request will send it back for admin review.
                Expenses can only be claimed after the budget has been approved.
            </div>

            <div class="workflow-list">
                <div class="workflow-item">
                    <strong>1. Review admin comment</strong>
                    <span>Adjust the amount, purpose or justification based on feedback.</span>
                </div>

                <div class="workflow-item">
                    <strong>2. Resubmit budget request</strong>
                    <span>The request will return to pending approval status.</span>
                </div>

                <div class="workflow-item">
                    <strong>3. Wait for approval</strong>
                    <span>Only approved budget requests can be used for expense claims.</span>
                </div>
            </div>
        </div>
        ...
        <div class="budget-card">
                ...
                <select name="campaign_id" id="campaignSelect" required>
                    ...
                    @foreach($campaigns as $campaign)
                        ...
                        <option value="{{ $campaign->id }}"
                                ...
                                {{ old('campaign_id', $fundAllocation->campaign_id) == $campaign->id ? 'selected' : '' }}>
                            {{ $campaign->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="premium-grid">
                <div class="premium-field">
                    ...
                    <input type="number"
                           ...
                           min="1"
                           step="0.01"
                           value="{{ old('amount', $fundAllocation->amount) }}"
                           required>
                </div>
                    ...
                    <select name="purpose_id" required>
                        ...
                        @foreach($purposes as $purpose)
                            <option value="{{ $purpose->id }}"
                                {{ old('purpose_id', $fundAllocation->purpose_id) == $purpose->id ? 'selected' : '' }}>
                                {{ $purpose->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="premium-field">
                ...
                <textarea name="note"
                          rows="5"
                          placeholder="Explain why this budget is needed and how it will support the campaign...">{{ old('note', $fundAllocation->note) }}</textarea>
            </div>

            <div class="submit-panel">
                <div>
                    <h3>Resubmit budget request?</h3>
                    <p>Admin will review the revised budget before it can be used for claims.</p>
                </div>
